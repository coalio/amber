...
- For a real user chat, the runtime sends a brief receipt before `CodexRunTask` starts the worker. After success, return `action=ignore`; the runtime has already acknowledged the request. For synthetic `linear_task_list` frames, also keep `action=ignore`.
