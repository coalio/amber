...
def test_reply_can_rewrite_memory_for_exact_profile(tmp_path) -> None:
    ...
    original_entry = memory_store.write_bad_memory(
        "user-123",
        "Casey",
        "Casey was hostile and kept trying to start fights.",
        source_message_id=401,
    )
    memory_card = memory_store.expand("user-123", "Casey", [original_entry.memory_id])[0]
    session = ConversationSession(
        session_id="sess_rewrite",
        chat_id=1001001001,
        last_updated_at=utc_now(),
        recent_messages=[_message(501, "user-123", "Casey", "look, i know i was annoying earlier")],
    ...
    context_layer.handle_semantic_decision(
        SemanticDecisionMadeEvent(
            ...
            payload=SemanticDecisionPayload(
                ...
                rewritten_memory_text="Casey used to be combative, but later apologized and cooled off.",
                rewritten_memory_tags=["past_conflict", "apology", "cooled_off"],
            ),
        )
    )

    rewritten_entry = memory_store._iter_memories("user-123", "Casey")[-1]

    assert rewritten_entry.memory_id == memory_card.memory_id
    assert rewritten_entry.text == "Casey used to be combative, but later apologized and cooled off."
...
def test_ignore_can_forget_memory_for_exact_profile(tmp_path) -> None:
    context_layer, memory_store = _build_context_layer(tmp_path)
    casey_entry = memory_store.write_bad_memory(
        "user-123",
        "Casey",
        "Casey kept dragging the chat into pointless fights.",
        source_message_id=410,
    )
    dee_entry = memory_store.write_bad_memory(
        "user-456",
        "Dee",
        "Dee likes clean code and short answers.",
        source_message_id=411,
    )
    casey_card = memory_store.expand("user-123", "Casey", [casey_entry.memory_id])[0]
    session = ConversationSession(
        session_id="sess_forget",
        chat_id=1001001001,
        last_updated_at=utc_now(),
        recent_messages=[_message(502, "user-123", "Casey", "thanks for letting that go")],
        participant_names={"user-123": "Casey", "user-456": "Dee"},
        engaged_user_ids={"user-123", "user-456"},
        latest_trigger_message_id=502,
        memory_cards={casey_card.memory_id: casey_card},
        expanded_memory_ids={casey_card.memory_id},
    )
    context_layer._active_session = session
    ...
    context_layer.handle_semantic_decision(
        SemanticDecisionMadeEvent(
            ...
            payload=SemanticDecisionPayload(
                ...
                target_memory_id=casey_card.memory_id,
                target_memory_sender_id="user-123",
            ),
        )
    )

    assert memory_store._iter_memories("user-123", "Casey") == []
    assert memory_store._iter_memories("user-456", "Dee")[-1].memory_id == dee_entry.memory_id
    assert casey_card.memory_id not in session.memory_cards
    assert casey_card.memory_id not in session.expanded_memory_ids
...
def test_expand_memory_uses_memory_owner_from_card_not_latest_trigger_sender(tmp_path) -> None:
    ...
    entry = memory_store.write_bad_memory(
        "user-123",
        "Casey",
        "Casey had a rough patch months ago, but it eventually settled down.",
        source_message_id=420,
    )
    casey_card = memory_store.expand("user-123", "Casey", [entry.memory_id])[0]
    session = ConversationSession(
        session_id="sess_expand_owner",
        chat_id=1001001001,
        last_updated_at=utc_now(),
        recent_messages=[_message(503, "user-456", "Dee", "what do you think now?")],
        participant_names={"user-123": "Casey", "user-456": "Dee"},
        engaged_user_ids={"user-123", "user-456"},
    ...
    context_layer.handle_semantic_decision(
        SemanticDecisionMadeEvent(
            ...
            payload=SemanticDecisionPayload(
                ...
                confidence=0.7,
                referenced_memory_ids=[casey_card.memory_id],
            ),
        )
    )

    assert casey_card.memory_id in session.expanded_memory_ids
    assert frames
    expanded_card = next(memory for memory in frames[-1].payload.relevant_memories if memory.memory_id == casey_card.memory_id)
    assert expanded_card.owner_sender_id == "user-123"
    assert expanded_card.owner_sender_name == "Casey"
